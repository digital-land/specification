...
defaults = {
    ...
    "fund": "green-belt-reviews",
    "intervention": "plan-making",
    "documentation-url": "https://www.gov.uk/government/publications/funding-to-support-local-authorities-with-the-costs-of-local-plan-delivery-and-green-belt-reviews-successful-local-authorities/funding-to-support-local-authorities-with-the-costs-of-green-belt-reviews-successful-local-authorities",
}
...
pq = PyQuery(filename="var/cache/green-belt-reviews.html")
...
for tr in pq("tr").items():
    ...
    if cols:
        ...
        row["amount"] = str(cols[1].text).replace("£", "").replace(",", "").strip()
        w.writerow(row)
