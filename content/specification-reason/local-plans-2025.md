---
end-date: ''
entry-date: '2026-03-24'
guidance: "You must follow the approved data standard to meet the requirements of the [Planning Data (England) Regulations 2026](#TBC),\nwhich will help you to meet the requirements of the \n[Town and Country Planning (Local Planning) (England) Regulations 2026](https://www.legislation.gov.uk/uksi/2026/186/contents/made). \n\nThis applies to the following plans that you create or update using the new plan-making system:  \n\n* local plans  \n* minerals and waste plans  \n* supplementary plans \n"
name: New local plans approved data standard
notes: ''
reference: local-plans-2025
start-date: ''
---
